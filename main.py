...
async def notify_admin_blocked(user: types.User, action: str):
    full_name = user.full_name
    username = f"@{user.username}" if user.username else "-"
    text = (
        "🚫 *Заблокований користувач намагався скористатись ботом*\n\n"
...
@dp.message()
async def blocked_guard(message: types.Message):
    if not is_blocked(message.from_user.id):
        return  # ⬅️ ОБОВʼЯЗКОВО

    await notify_admin_blocked(
        message.from_user,
        action=f"Текст / кнопка: {message.text}"
    )
...
@dp.callback_query()
async def blocked_callback_guard(callback: CallbackQuery):
    if not is_blocked(callback.from_user.id):
        return

    await notify_admin_blocked(
        callback.from_user,
        action=f"Inline кнопка: {callback.data}"
    )
