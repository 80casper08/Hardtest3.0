...
async def start_quiz(message: types.Message, state: FSMContext):
    ...
    if current_section is not None:
        await message.answer("❗ Завершіть поточний тест перед початком нового.")
        return
    ...
    await state.update_data(current_section=category, category=category, question_index=0, selected_options=[], wrong_answers=[], questions=questions)

    # Приховуємо кнопки меню (видаляємо клавіатуру)
    await message.answer("Тест розпочато. Відповідайте на питання:", reply_markup=types.ReplyKeyboardRemove())
...
async def restart_test(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    if data.get("restart_used"):
        await callback.message.answer("⛔ Ви надто часто проходите цей розділ.\nНатисніть /start або оберіть інший розділ.")
        await callback.answer()
        return

    # Позначаємо, що кнопку вже використовували
    await state.update_data(restart_used=True)

    category = data.get("category")
    if not category or category not in sections:
        await callback.message.answer("⚠️ Щось пішло не так. Спробуйте /start")
        return
    ...
    await callback.message.answer("🔁 Тест перезапущено:", reply_markup=types.ReplyKeyboardRemove())
    await callback.answer()
